...
RUN apt-get -y update && \
  ...
  apt-get -y install python-is-python3 git build-essential ffmpeg --no-install-recommends && \
  python3 -m pip install --upgrade pip && \
  pip install pdm && \
  pdm config python.use_venv true && \
  apt-get -y install zlib1g g++ freeglut3-dev libx11-dev libxmu-dev libxi-dev libglu1-mesa libglu1-mesa-dev libfreeimage-dev zsh gdb && \
